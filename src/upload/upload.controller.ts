...
export class UploadController {
    ...
    if (!allowedMimes.includes(file.mimetype)) {
      return cb(
        new BadRequestException(
          `Only accept file: ${allowedMimes.join(', ')}`,
        ),
        false,
      );
    }
  ...
  async uploadDocument(
    ...
    return this.uploadService.sendDocumentByBuffer(
      file.buffer,
      file.originalname,
      caption,
    );
  ...
  async uploadManyDocuments(
    ...
    return this.uploadService.sendDocumentsByBuffers(files, caption);
  }
}
